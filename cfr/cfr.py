...
from .mlp import MLP
from .utils import mmd_rbf, mmd_lin
    ...
    def fit(
        ...
        for epoch in range(self.cfg["epochs"]):
            ...
            for (x, y, z) in dataloader:
                ...
                if self.cfg["alpha"] > 0.0 and len(_t_id) > 0 and len(_c_id) > 0:
                    ...
                    loss = loss + ipm * self.cfg["alpha"]
                    epoch_ipm.append(float(ipm.detach().cpu().numpy()))

                y_pred = y_hat.detach().cpu().numpy().reshape(-1)
                y_true = y.reshape([-1, 1]).detach().cpu().numpy().reshape(-1)
                if np.all(np.isfinite(y_pred)) and np.all(np.isfinite(y_true)):
